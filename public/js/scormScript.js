...
setInterval(function(){
	...
	for(var i=0;i<elems.length;i++){
		if(elems[i].alt.startsWith("MobaCloudOpC") && Number(elems[i].dataset.initizialized)!="1" && document.getElementById("mobacloud")){
			...
			elems[i].setAttribute("onclick","console.log('Sent message to parent');parent.postMessage('LaunchSimulation$https://operators.modeller.cloud/lmsLogin/"+encodeURIComponent(document.getElementById('mobacloud').dataset.hostname)+"/"+encodeURIComponent(document.getElementById('mobacloud').dataset.id)+"?exercisename="+encodeURIComponent(exerciseName)+"')");
		}
	}	
},1000);
...
function generateMobaCloudIframe(studentIdF,hostnameF){
	if(!document.getElementById("mobacloud-wrap")){
		...
		iFrameDiv.setAttribute("style","display:none;position:relative;overflow:hidden;width:100vw;height:100vh;background:rgb(37,78,118);background: linear-gradient(0deg, rgb(20, 42, 64), rgb(37, 78, 118));")
			...
			styleDiv.innerHTML = "#mobacloud-wrap iframe{width:100vw;height:100vh;position: absolute;top: 0;left: 0;transform-origin: top left;border:1px solid rgb(37,78,118);box-shadow:2px 2px 2px rgba(0,0,0,0.8);}";
